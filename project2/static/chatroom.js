document.addEventListener('DOMContentLoaded', () => {
    ...
    socket.on('cast message', data => {
        if (data["chat_id"] === localStorage.chat_id) {
            ...
            // Used innerHTML instead of innerText to keep it raw
            if (data["selection"].includes("https://www.youtube.com/embed/"))
            {
                li.innerHTML = `<strong>${data["user_name"]}</strong> : <iframe src=${data["selection"]} width="553" height="480" frameborder="0" allowfullscreen></iframe> <small>${data["time"]}</small>`;
            }
            else

            // <iframe src="https://www.youtube.com/embed/d73tiBBzvFM" width="853" height="480" frameborder="0" allowfullscreen></iframe>
